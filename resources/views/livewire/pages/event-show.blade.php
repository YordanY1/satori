<section class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    @if (!empty($event['cover']))
        <img src="{{ $event['cover'] }}" alt="{{ __('events.cover', ['title' => $event['title']]) }}"
            class="w-full h-auto rounded-2xl shadow mb-6">
    @endif
    ...
    @if (!empty($event['program']))
        <h2 class="text-xl font-semibold mb-2">{{ __('events.program') }}</h2>
    ...
    @if (!empty($event['video_url']))
        <h2 class="text-xl font-semibold mt-6 mb-2">{{ __('events.video') }}</h2>
        <div class="aspect-video rounded-xl overflow-hidden shadow">
            <iframe class="w-full h-full" src="{{ $event['video_url'] }}?rel=0&modestbranding=1"
                title="{{ __('events.video_title', ['title' => $event['title']]) }}" loading="lazy"
